...
namespace WebAPIBedminton.Controllers
{
    ...
    public class ProductController : ApiController
    {
        ...
        {
            IEnumerable<ProductDTO> ls;
            if (number == null)
                ls = new ProductDAO().List();
            else
                ls = new ProductDAO().List().Take((int)number);
            if (ls == null)
            {
                return NotFound();
            }
        ...
        {
            var b = new ProductDAO().Take(id); // Lay phan tu voi id tuong ung
            if (b == null)
            {
                return NotFound();
            }
        ...
        public IHttpActionResult Post([FromUri]JObject jsonResult)
        {
            ...
            var x = JsonConvert.DeserializeObject<ProductDTO>(jsonResult.ToString());
            var last = new ProductDAO().Add(x);
            return Ok(last);
        }
        #endregion
        ...
        public IHttpActionResult Put([FromBody]JObject jsonResult)
        {
            ...
            var x = JsonConvert.DeserializeObject<ProductDTO>(jsonResult.ToString()); // Ep kieu Json sang DTO
